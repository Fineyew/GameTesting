...
func run() -> void:
    ...
    app.queue_free()
    await process_frame
    print("GODOT_ONLINE_PASS: account, creation, two-player presence, movement, branching dialogue, quest, combat, rewards, reconnect, ordered Mara investigation, once-only rewards")
    quit()
