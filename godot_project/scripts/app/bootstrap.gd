...
func authenticate(registering: bool) -> void:
    ...
    if info.is_empty() or info.get("world_protocol",0) != 2 or info.get("story_protocol",0) != 1 or info.get("folio_protocol",0) != 1 or info.get("commerce_protocol",0) != 1 or info.get("item_use_protocol",0) != 1 or info.get("tidebeat_protocol",0) != 2:
