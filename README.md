...
The client has an auth screen, character creation screen, readable character/quest/inventory HUD, and a placeholder 3D Dawnreef field with player, NPC, and enemy markers. Move the player marker with WASD or arrow keys; contextual Talk/Fight buttons appear when near Mara or the Fog-Thorn enemy. Talk opens a short Mara dialogue with an accept-quest choice, and Fight opens a small combat panel with Glimmer Spark, Root Snare, and Tide Mend actions. It can register/login, create a character, enter the world, accept the starter quest, fight the starter enemy, save, logout, and display the current character state.
