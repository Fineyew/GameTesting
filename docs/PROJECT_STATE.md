...
TidebeatEffect supplies eight distinct opaque, shadow-free silhouettes: lantern arcs/
marks, reed cages/shields, tide rings/lances, a crossed Brace and gathering motes. It uses
at most 16 meshes and 4,000 triangles per action, no particles/transparency/multipass.
Two original pluck/sweep cues distinguish Rootbinding and Tideseaming; the complete
sound source set is now 13 assets / 525,467 bytes. Existing spell IDs/rules and saves remain.
TidebeatPlayback consumes an already-confirmed receipt, restores camera/HUD on completion,
GUI skip or backgrounding, and suppresses repeated recent receipts. No presentation path
sends a combat command or awards rewards. Short playback speeds effects and rig actions
by 2.6; reduced motion keeps still silhouettes and the gameplay camera. Sound captions
remain visible during the cast. Combat buttons show target/cost/effect; enemy intent and
Beacon mark remain readable as text. New panels reset scroll to show their first control.

Local actual-engine checks cover eight shapes/effect fixtures, GUI skip, duplicate receipt,
static reduced mode, shorter rig timing, pause/resume and unchanged character state.
Fixtures are generated by the real Python CombatEngine without a player store. Rendered
low/high action views pass: up to 54 / 89 calls and 75,282 / 147,986 primitives. The first
high-shadow comparison exposed 353,278 primitives; one directional cascade over 16 metres
replaces four long cascades, preserving the existing 150k budget. Window/mixer cleanup
releases player ownership before draining asynchronous playback; the final render has
no engine errors. A 12.23-second actual-engine H.264/AAC review movie includes all eight
presentations and the short/reduced modes. These are software captures, not phone FPS.
The extended real-API route passes all eight actions, saved-state checks after skips,
folio/rewards/reconnect, supplies and two-player terrain. Its first fixture sequence
...
Code `928054e54530dc4c3cd5dec5c83ec5701e6107a1` passes all three jobs in
[run 34773533442](https://github.com/Fineyew/GameTesting/actions/runs/34773533442).
Original source synthesis produces a 24-second Dawnreef motif, related Tidebeat loop,
16-second wind bed and eight cues (11 assets / 472,459 source bytes). Soundscape owns
Master, Music, Ambience, Movement, Spells, Creatures and UI controls, smooth zone/combat/
panel mixing, eight capped voices, a -1 dB limiter, optional softer effects and captions.
Debounced preference saves preserve graphics/network settings. Background pauses loops
and drops one-shots; resume cannot replay old cues. Actual gait contacts and confirmed
combat presentation drive cues. No external recording/voice/sample-pack license or cost.

Fresh import, real mixer loop wrap/mute/overlap, GUI controls/config preservation, full
Godot/creator/motion/API, terrain (32 cases / 740 motions), 36 impairments and isolated
authoring pass. Backend: 214 tests and 30 subtests, including 10 PostgreSQL tests and both
migrations. Android: nine harness tests, export/signature verification, render/movie and
native touch/visible-resume/creator/settled online entry pass. Retained native log shows
actual touch footsteps and new pause/resume markers across Home; this is lifecycle code
evidence, not exported phone speaker/headphone listening. Actual engine audio is recorded.

Verified ARM64 **0.2.11/code 13**, 29,631,214 bytes, tested merge
`11af7bd5cb63d452c8efd89229484cfe1b4c0c09`. [APK artifact 10322324452](https://github.com/Fineyew/GameTesting/actions/runs/34773533442/artifacts/10322324452),
native 10323092128 / render 10323201023, retained through 2026-12-12. Downloaded ZIP/APK
CRC/SHA, ARM64-only libraries, version/code, compiled audio controls, source manifest and
retained v2/v3 signing report checked. APK SHA256:
`d07349061751d3de566174aa2621b8d98b7c41f1f3c87c79cd077ca27d394ec2`.
Town 71 calls / 79,014 primitives / 15,325,211 texture bytes; creator 75, terrain 52;
32-rig crowd 200 calls / 118,988 primitives. APK and engine audio preview are saved.
Phone listening, owner acceptance and sustained mobile measurements remain open.
