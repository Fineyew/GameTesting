...
- Present confirmed results with target/cost/effect text, enemy intent/mark, GUI skip,
  2.6× effect/rig playback and static reduced motion without a camera cut.
- Add real-engine fixtures generated by CombatEngine, receipt/skip/lifecycle checks,
  low/high captures and an actual-engine movie with audio. Preserve rules, IDs and saves.
- Fix high-quality shadow cost within the original 150k budget using one 16m cascade;
  release audio player ownership before final mixer drain after long rendered sequences.
- Local render/headless and full eight-action API route pass; Android CI pending for 0.2.12/code 14.

## 2026-09-13 — M1.9 audio validated

- Source 928054e / run 34773533442 passes all backend, Godot and Android gates.
- Retain verified ARM64 0.2.11/code 13, artifact 10322324452, native 10323092128
  and render 10323201023, plus the actual engine mix. PROJECT_STATE records exact hashes.
- Native touch emits footsteps; Home/resume emits fresh audio lifecycle markers.
  Physical listening, owner review and sustained device measurements remain open.
